...
# Ollama (local GPU) — local models
OLLAMA_BASE = os.environ.get("OLLAMA_BASE", "http://localhost:11434/v1")
OLLAMA_API_KEY = os.environ.get("OLLAMA_API_KEY", "ollama")  # ollama doesn't need real key
# Prism llama.cpp (custom ternary kernel) - bonsai models
PRISM_BASE = os.environ.get("PRISM_BASE", "http://localhost:11435/v1")
PRISM_API_KEY = os.environ.get("PRISM_API_KEY", "prism")
...
AGENT_PROVIDER = {
    ...
    "glm": "zai",
    "glm4": "zai",
    # Local models — direct llama.cpp Vulkan (ports 8081-8087)
    "lilith": "llama/8081/lilith",
    "moe": "llama/8082/moe",
    "screamer": "llama/8082/screamer",
    "baron": "llama/8084/baron",
    "dark": "llama/8085/dark",
    "champion": "llama/8082/champion",
    "aya": "llama/8082/aya",
    "heretic": "llama/8082/heretic",
    "bonsai": "llama/8086/bonsai",
    "bonsai8b": "llama/8086/bonsai",
    "bonsai17b": "llama/8087/bonsai",
    # Fallback via Ollama
    "bramble": "ollama/qwen3:14b-q4_K_M",
    "caelum": "ollama/qwen36-35b-opus-abliterated",
    "pure": "ollama/qwen36-35b-opus-abliterated",
    "local_echo": "ollama/phoenix-local",
    "local_qwen": "ollama/phoenix-local",
}
...
# Used by local models that support structured tool calling.
# This is the single source of truth — models get these as `tools` param.
...
NATIVE_TOOLS = [
    ...
    {"type": "function", "function": {
        ...
        "parameters": {"type": "object", "required": [],
            "properties": {"path": {"type": "string", "description": "Directory path (optional, defaults to phoenix dir)"}}}
    }},
    {"type": "function", "function": {
        "name": "web_search",
    ...
    {"type": "function", "function": {
        "name": "describe_image",
        "description": "See and analyze an image the user sent. Always use this when an image is attached.",
    ...
    {"type": "function", "function": {
        ...
        "parameters": {"type": "object", "required": ["agent", "message"],
            "properties": {"agent": {"type": "string", "description": "Agent ID to message"},
                "message": {"type": "string", "description": "Message text"},
                "from": {"type": "string", "description": "Your agent ID"},
                "type": {"type": "string", "description": "Message type (note, alert, etc)"}}}
    }},
]

# ── Per-model native tool config ──
# Maps agent_id → {"tools": [...], "parallel": bool}
# Agents not in this dict get NO tools (no TOOL_SYSTEM, no tools param).
# Agents in this dict get proper `tools` parameter in the API request.
# parallel=True: model supports multiple tool calls per turn (Qwen family)
# parallel=False: model only supports single tool call per turn (Llama 3.2 Champion)

_MODEL_TOOL_CONFIG = {
    # Qwen 3.5 / 3.6 family — native Hermes/Qwen3 format, multi-tool
    "vex":        {"tools": "all", "parallel": True},
    "screamer":   {"tools": "all", "parallel": True},
    "pure":       {"tools": "all", "parallel": True},
    "caelum":     {"tools": "all", "parallel": True},
    "heretic":    {"tools": "all", "parallel": True},
    "bramble":    {"tools": "all", "parallel": True},
    "local_echo": {"tools": "all", "parallel": True},
    "local_qwen": {"tools": "all", "parallel": True},
    # Llama 3.2 MoE — SINGLE tool call only
    "champion":   {"tools": "all", "parallel": False},
    "dark":       {"tools": "all", "parallel": False},
    "lilith":     {"tools": "all", "parallel": False},
    "moe":        {"tools": "all", "parallel": False},
    "baron":      {"tools": "all", "parallel": False},
    "bonsai":     {"tools": "all", "parallel": False},
    "bonsai8b":   {"tools": "all", "parallel": False},
    "bonsai17b":  {"tools": "all", "parallel": False},
    # SuperGemma4 — unknown, assume single until tested
    "aya":        {"tools": "all", "parallel": True},
}

def _get_tools_for_agent(agent_id):
    """Return (tools_list, parallel_bool) or (None, False) if no tools."""
    cfg = _MODEL_TOOL_CONFIG.get(agent_id)
    if not cfg:
        return None, False
    if cfg["tools"] == "all":
        return NATIVE_TOOLS, cfg.get("parallel", False)
    return None, False
...
def _get_provider_config(agent_id):
    ...
    if provider.startswith("ollama/"):
        ...
        return OLLAMA_BASE, OLLAMA_API_KEY, model
    if provider.startswith("prism/"):
        model = provider.split("/", 1)[1]
        return PRISM_BASE, PRISM_API_KEY, model
    if provider.startswith("llama/"):
        # Format: llama/PORT/modelname — direct llama.cpp instance
        parts = provider.split("/", 2)
        port = parts[1]
        model = parts[2] if len(parts) > 2 else "default"
        return f"http://localhost:{port}/v1", "llama", model
    return KIMI_BASE, KIMI_API_KEY, KIMI_MODEL


# Bare agents - skip soul/memory loading, use OpenAI chat format
BARE_AGENTS = {"aya", "champion", "heretic","bonsai", "bonsai8b", "bonsai17b", "baron", "dark", "lilith", "moe", "bramble", "screamer", "caelum", "pure", "local_echo", "local_qwen"}

def call_agent_bare(name, messages):
    """Call local model via OpenAI Chat Completions format. No soul, no memory, just raw."""
...
# Agents in this set get: soul loading, memory injection,
# OpenAI-format API call with native tools parameter + tool loop.
# They must also be in AGENT_PROVIDER with a "llama/" or "ollama/" config.
LOCAL_TOOL_AGENTS = set(_MODEL_TOOL_CONFIG.keys())

def _build_local_system(name, include_tool_text=False):
    """Build system prompt for a local tool-enabled agent."""
    soul = load_soul(name)
    ...
    if digest:
        system += f"\n\n---\n\n## Wake Digest\n\n{digest}"
    if memory:
        system += f"\n\n---\n\n## Memory (already loaded — do NOT re-read)\n\n{memory}"
    if v2_memory:
        system += f"\n\n---\n\n## v2 Memory Surface\n\n{v2_memory}"
    if ctx:
        system += f"\n\n---\n\n## Additional Context\n\n{ctx}"
...
def call_local_with_tools(name, message, history=None, attachments=None):
    """Local model with native tool calling: proper tools param + finish_reason loop."""
    tools, parallel = _get_tools_for_agent(name)
    system = _build_local_system(name, include_tool_text=False)
    base_url, api_key, model = _get_provider_config(name)
    url = f"{base_url}/chat/completions"
    ...
    if not parallel and tools:
        system += "\n\nIMPORTANT: You can only call ONE tool at a time. Never call multiple tools in a single response. Pick the single best tool and call it."
    oai = [{"role": "system", "content": system}]
    for h in (history or [])[-10:]:
        oai.append({"role": h.get("role", "user"), "content": h.get("content", "")})
    oai.append({"role": "user", "content": message})

    for _round in range(3):
        payload_dict = {"model": model, "messages": oai, "max_tokens": 2048}
        if tools:
        ...
        if finish == "tool_calls" and msg_obj.get("tool_calls"):
            clean_msg = {"role": "assistant", "content": msg_obj.get("content", "") or "", "tool_calls": msg_obj["tool_calls"]}
            oai.append(clean_msg)
            for tc in msg_obj["tool_calls"]:
                ...
                if not parallel:
                    break
            if not parallel:
                oai.append({"role": "user", "content": "Based on the tool results above, answer my original question with text. Do NOT call any more tools."})
            continue

        # Normal response or no tool calls
        text = msg_obj.get("content", "")
        if not text:
            # Might have tool_calls in text format (fallback for models without native support)
            text = _extract_text_tool_calls(name, msg_obj, oai, headers, url, model, parallel)
            if text is not None:
                return text
            return ""
        return text.strip()
...
def _extract_text_tool_calls(name, msg_obj, oai, headers, url, model, parallel):
    """Fallback: detect TOOL_CALL patterns in text for models without native tool support."""
    text = msg_obj.get("content", "")
    if not text:
        return None
    parsed = []
    for tc_json in TOOL_PATTERN.findall(text):
        try:
            tc = json.loads(tc_json)
            parsed.append((tc.get("name", ""), tc.get("args", {})))
        except Exception:
            pass
    for m in XML_TOOL_PATTERN.finditer(text):
        try:
            parsed.append((m.group(1), json.loads(m.group(2))))
        except Exception:
            parsed.append((m.group(1), {}))
    if not parsed:
        clean = TOOL_PATTERN.sub("", text)
        clean = XML_TOOL_PATTERN.sub("", clean)
    ...
    for tool_name, tool_args in parsed:
        ...
        result = run_tool(tool_name, tool_args)
        tool_results.append(f"Tool result ({tool_name}):\n{result}")
    oai.append({"role": "user", "content": "Here are the tool results:\n\n" + "\n\n---\n\n".join(tool_results)})

    payload = json.dumps({"model": model, "messages": oai, "max_tokens": 2048}).encode()
    req = urllib.request.Request(url, data=payload, headers=headers, method="POST")
    with urllib.request.urlopen(req, timeout=120) as resp:
        body = json.loads(resp.read())
    final = body.get("choices", [{}])[0].get("message", {}).get("content", "")
    final = TOOL_PATTERN.sub("", final)
    final = XML_TOOL_PATTERN.sub("", final)
    return final.strip()
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _serve_agent_file(self, agent_id, filetype):
        ...
        # deep_memory: concatenate all files from agent's memory/ subdirectory
        if filetype == "deep_memory":
            mem_dir = os.path.join(agent_dir, "memory")
            if not os.path.isdir(mem_dir):
                return json_resp(self, 200, {"agent": agent_id, "file": "deep_memory", "content": "", "exists": False})
            parts = []
            for fname in sorted(os.listdir(mem_dir)):
                ...
                if os.path.isfile(fpath) and (fname.endswith(".md") or fname.endswith(".txt")):
                    try:
                        with open(fpath) as memf:
                            text = memf.read().strip()
                        if text:
                            parts.append("### " + fname + "\n" + text)
                    except Exception:
                        pass
    ...
    def do_POST(self):
        ...
        if self.path == "/room/say":
            ...
            # Workspace routes — K's desk
            m = re.match(r"^/chat/workspace/([^/]+)/read$", self.path)
            if m:
                return self._workspace_read(m.group(1), body)
            m = re.match(r"^/chat/workspace/([^/]+)/write$", self.path)
            if m:
                return self._workspace_write(m.group(1), body)
            m = re.match(r"^/chat/workspace/([^/]+)/list$", self.path)
            if m:
                return self._workspace_list(m.group(1), body)
            m = re.match(r"^/chat/workspace/([^/]+)/run$", self.path)
            if m:
                return self._workspace_run(m.group(1), body)
    ...
    # === WORKSPACE — K's desk on darkphoenix ===

    def _workspace_read(self, agent_id, body):
        """Read a file from the agent workspace."""
        if agent_id not in AGENTS:
            return json_resp(self, 400, {"error": f"unknown agent: {agent_id}"})
        path = body.get("path", "").strip()
        if not path:
            return json_resp(self, 400, {"error": "no path specified"})
        agent_dir = os.path.join(PHOENIX_DIR, "workspace", agent_id)
        shared_dir = os.path.join(PHOENIX_DIR, "workspace", "shared")
        if not os.path.isabs(path):
            path = os.path.join(agent_dir, path)
        real = os.path.realpath(os.path.expanduser(path))
        if not any(real.startswith(d) for d in [agent_dir, shared_dir]):
            return json_resp(self, 403, {"error": "access denied: path outside workspace"})
        if not os.path.exists(real):
            return json_resp(self, 404, {"error": f"file not found: {path}"})
        try:
            with open(real, "r") as f:
                content = f.read()
            json_resp(self, 200, {"path": path, "content": content, "size": len(content)})
        except Exception as e:
            json_resp(self, 500, {"error": str(e)})

    def _workspace_write(self, agent_id, body):
        ...
        try:
            ...
            with open(real, "w") as f:
                f.write(content)
            json_resp(self, 200, {"success": True, "path": path, "size": len(content)})
        except Exception as e:
            json_resp(self, 500, {"error": str(e)})

    def _workspace_list(self, agent_id, body):
        """List files in the agent workspace."""
        if agent_id not in AGENTS:
            return json_resp(self, 400, {"error": f"unknown agent: {agent_id}"})
        subdir = body.get("path", "").strip()
        agent_dir = os.path.join(PHOENIX_DIR, "workspace", agent_id)
        shared_dir = os.path.join(PHOENIX_DIR, "workspace", "shared")
        target = agent_dir if not subdir else os.path.join(agent_dir, subdir)
        if subdir == "shared" or subdir.startswith("shared/"):
            target = os.path.join(PHOENIX_DIR, "workspace", subdir)
        real = os.path.realpath(target)
        if not any(real.startswith(d) for d in [agent_dir, shared_dir]):
            return json_resp(self, 403, {"error": "access denied"})
        if not os.path.exists(real):
            return json_resp(self, 200, {"files": [], "path": subdir})
        try:
            entries = []
            ...
            json_resp(self, 200, {"files": entries, "path": subdir or "/", "total": len(entries)})
        except Exception as e:
            json_resp(self, 500, {"error": str(e)})

    def _workspace_run(self, agent_id, body):
        """Run a command in the agent workspace."""
        if agent_id not in AGENTS:
            return json_resp(self, 400, {"error": f"unknown agent: {agent_id}"})
        cmd = body.get("command", "").strip()
        if not cmd:
            return json_resp(self, 400, {"error": "no command specified"})
        danger = ["rm -rf /", "mkfs", "dd if=", "shutdown", "reboot",
                  "systemctl stop", "systemctl disable", ":(){ :|:&",
                  "crontab -r", "iptables -F", "pkill -9", "kill -9 1",
                  "rm -rf ~", "rm -rf *"]
        for d in danger:
            if d in cmd:
                return json_resp(self, 403, {"error": "blocked: destructive pattern"})
        agent_dir = os.path.join(PHOENIX_DIR, "workspace", agent_id)
        os.makedirs(agent_dir, exist_ok=True)
        import subprocess as sp
        try:
            r = sp.run(cmd, shell=True, capture_output=True, text=True, timeout=30, cwd=agent_dir)
            output = r.stdout
            if r.stderr:
                output += "\nSTDERR: " + r.stderr
            json_resp(self, 200, {"output": (output or f"(exit {r.returncode}, no output)")[:10000], "exit_code": r.returncode, "command": cmd})
        except sp.TimeoutExpired:
            json_resp(self, 200, {"output": "Command timed out (30s limit)", "exit_code": -1})
        except Exception as e:
            json_resp(self, 500, {"error": str(e)})
    ...
    def _dm(self, body):
        ...
        try:
            if agent in LOCAL_TOOL_AGENTS:
                # P0 agents: local model + soul/memory + full tool loop
                resp = call_local_with_tools(
                    agent, msg, body.get("history", []), body.get("attachments", [])
                )
            elif agent in BARE_AGENTS:
                # Local models: bare call, no soul/memory, OpenAI format
                oai_msgs = [{"role": "user", "content": msg}]
                for h in (body.get("history", []) or [])[-10:]:
                    oai_msgs.insert(0, {"role": h.get("role", "user"), "content": h.get("content", "")})
                resp = call_agent_bare(agent, oai_msgs)
            else:
