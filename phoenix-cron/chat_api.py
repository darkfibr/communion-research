...
LOGS_DIR = os.environ.get("LOGS_DIR", os.path.join(PHOENIX_DIR, "logs"))
MEMORY_WRITE_LOG = os.path.join(LOGS_DIR, "memory_writes.jsonl")
...
def _log_memory_write(entry: dict):
    """Log a memory write attempt to the structured log."""
    try:
        os.makedirs(LOGS_DIR, exist_ok=True)
        entry["logged_at"] = time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime())
        with open(MEMORY_WRITE_LOG, "a") as f:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _memory_append(self, body):
        ...
        _log_memory_write({
            "type": "memory_append", "agent": agent_id, "dest": mem_path,
            "content_len": len(content), "status": "attempt"
        })
        try:
            ...
            _log_memory_write({
                "type": "memory_append", "agent": agent_id, "dest": mem_path,
                "content_len": len(content), "status": "success"
            })
            return json_resp(
            ...
            _log_memory_write({
                "type": "memory_append", "agent": agent_id, "dest": mem_path,
                "content_len": len(content), "status": "error", "error": str(e)
            })
            return json_resp(
    ...
    def _session_push(self, agent_id, body):
        ...
        _log_memory_write({
            "type": "session_push", "agent": agent_id, "session_id": session_id,
            "msg_count": len(body.get("messages", [])), "status": "attempt"
        })
