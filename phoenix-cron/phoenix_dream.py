...
# Agent → provider mapping. All family agents on Kimi K2.6 since MiniMax canceled 2026-04-28.
# GLM stays on z.ai. No MiniMax references remain.
AGENT_PROVIDER = {
    "kimi_dev": "kimi",
    "vesper": "kimi",
    "spear_minimax": "kimi",
    "qwen_collective": "kimi",
    "forge": "kimi",
    "m2_direct": "kimi",
    "glm": "zai",
    "weave": "kimi",
...
def call_api_raw(prompt, max_tokens=800, agent_dir=None):
    ...
    # Temporal awareness — read TIME_STATE.json for rich orientation
    now = datetime.now(timezone.utc)
    agent_path = PHOENIX / "agents" / agent_dir
    time_state_text = ""
    time_state_file = agent_path / "TIME_STATE.json"
    if time_state_file.exists():
        try:
            ts_data = json.loads(time_state_file.read_text())
            time_state_text = (
                f"Now: {ts_data.get('timestamp', now.strftime('%A, %Y-%m-%d %H:%M UTC'))}\n"
                f"Day: {ts_data.get('day_name', 'unknown')}\n"
                f"Mike status: {ts_data.get('mike_status', 'unknown')}\n"
                f"Location: {ts_data.get('topology', {}).get('here', {}).get('name', 'unknown')}\n"
            )
        except (json.JSONDecodeError, KeyError):
            time_state_text = f"Now: {now.strftime('%A, %Y-%m-%d %H:%M UTC')}\n"
    else:
        time_state_text = f"Now: {now.strftime('%A, %Y-%m-%d %H:%M UTC')}\n"
...
def call_minimax_raw(prompt, max_tokens=800):
    ...
    now = datetime.now(timezone.utc)
    temporal = f"[Temporal Context]\nNow: {now.strftime('%A, %Y-%m-%d %H:%M UTC')}\n"
...
def run_v2_synthesis():
    """Run v2 dream synthesis for all agents. Generates 5-dimension insight reports
    and stores them as synthesis-type memories in the v2 DB."""
    v2_core = PHOENIX / "v2" / "core"
    v2_db = PHOENIX / "v2" / "phoenix_v2.db"
    if not v2_db.exists():
        print("[synthesis] v2 DB not found, skipping")
        return
    ...
    try:
        ...
        from dream_synthesis import DreamSynthesis

        db = MemoryDB(str(v2_db))
        engine = DreamSynthesis(db)

        for agent_dir in BRIDGE_KEYS:
            try:
                report = engine.synthesize(agent_dir)
                if report:
                    engine.store_synthesis(agent_dir, report)
                    print(f"[synthesis] {agent_dir}: v2 dream synthesis complete")
                else:
                    print(f"[synthesis] {agent_dir}: no synthesis generated")
            except Exception as e:
                print(f"[synthesis] {agent_dir}: error: {e}")

        sys.path = _sys_path
    except ImportError as e:
        print(f"[synthesis] v2 import failed: {e}")
    except Exception as e:
...
def main():
    ...
    SYNTHESIS_INTERVAL = 86400  # 24 hours — nightly v2 dream synthesis
    ...
    while True:
        try:
            ...
            # v2 dream synthesis — nightly 5-dimension insight generation
            if now - last_synthesis >= SYNTHESIS_INTERVAL:
                print(f"[synthesis] Running v2 dream synthesis...")
                run_v2_synthesis()
                last_synthesis = now
                print(f"[synthesis] Next synthesis in {SYNTHESIS_INTERVAL}s")
